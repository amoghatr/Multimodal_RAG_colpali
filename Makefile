.PHONY: clean-pycache clean-ruff-cache clean-mypy-cache clean-all \
		...
		create_collection frontend dev-full
...
# Run the Streamlit frontend (requires backend to be running on port 8000)
frontend:
...
dev-full:
	...
	@echo "Run 'make frontend' in another terminal to start the UI on port 8501"
	uv run uvicorn server:app \
		--host 0.0.0.0 \
		--port 8000 \
		--reload
